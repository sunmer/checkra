...
class ScreenCapture {
  ...
  private handleMouseUp = async (e: MouseEvent): Promise<void> => {
    ...
    if (width > 5 && height > 5) {
      // Only capture if selection is reasonably sized
      ...
      try {
        ...
        if (ctx) {
          ...
          ctx.drawImage(canvas, x + window.scrollX,
          // Source X (from full body capture)
          y + window.scrollY,
          // Source Y
          width,
          // Source Width
          ...
          // Source Height
          0,
          // Destination X (on cropped canvas)
          0,
          // Destination Y
          width,
          // Destination Width
  ...
  public startCapture(callback: (imageDataUrl: string | null) => void): void {
    ...
    try {
      ...
      document.addEventListener('mouseup', this.handleMouseUp, {
        capture: true,
        once: true
      }); // Use capture phase and once: true
